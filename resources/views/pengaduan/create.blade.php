        ...
        <form action="{{ route('pengaduan.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-lg border border-gray-200 space-y-6">
            ...
            <div>
                ...
                <textarea name="isi_pengaduan" id="isi_pengaduan" required rows="5"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#810000]">{{ old('isi_pengaduan') }}</textarea>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="file" name="foto" id="foto" accept="image/*"
                       class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#DDEDEE] file:text-[#810000] hover:file:bg-[#c9e0e1]">
            </div>
            <div class="flex justify-end">
                <button type="submit" class="bg-[#810000] text-white px-6 py-2 rounded-lg font-semibold hover:bg-[#a30000] transition">
                    Kirim Laporan
                </button>
            </div>
        </form>
